...
export default function TagsIndex({ tags, filters }) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (search !== filters.search) {
                router.get(route('admin.menu.tags.index'), { search }, {
                    preserveState: true,
                    preserveScroll: true,
                    only: ['tags']
                });
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-bold leading-tight text-gray-800">Tag Management</h2>}>
            ...
            <div className="mb-6 rounded-lg p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Link
                        href={route('admin.menu.tags.create')}
                ...
                <table className="w-full">
                    <thead className="bg-white">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left font-medium font-sans text-[14px] text-black uppercase tracking-wider">Menus Count</th>
                            <th className="px-6 py-3 text-left font-medium font-sans text-[14px] text-black uppercase tracking-wider">Created At</th>
                        ...
                        {tags.data.length === 0 ? (
                            <tr>
                                <td colSpan="6" className="px-6 py-4 text-center text-gray-500">
                                    No tags found. <Link href={route('admin.menu.tags.create')} className="text-blue-600 hover:underline">Create your first tag</Link>
                                </td>
                            </tr>
                        ) : (
                            tags.data.map((tag) => (
                                <tr key={tag.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap text-[14px] font-normal font-sans text-gray-900">{tag.id}</td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-[14px] font-normal font-sans text-gray-900">{tag.name}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-[14px] font-normal font-sans text-gray-500">{tag.slug}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-[14px] font-normal font-sans text-gray-500">{tag.menus_count}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center text-[14px] font-normal font-sans">
                                        <div className="flex items-center justify-center space-x-2">
                                            <Link
                                                href={route('admin.menu.tags.edit', tag.id)}
                                            ...
                                            <Link
                                                href={route('admin.menu.tags.destroy', tag.id)}
                                                method="delete"
                                                as="button"
