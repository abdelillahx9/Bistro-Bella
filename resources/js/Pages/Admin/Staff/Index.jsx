...
export default function StaffIndex({ staff, filters }) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (search !== filters.search || status !== filters.status) {
                router.get(route('admin.staff.index'), { search, status }, {
                    preserveState: true,
                    preserveScroll: true,
                    only: ['staff']
                });
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-bold leading-tight text-gray-800">Staff Management</h2>}>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full">
                            <thead className="bg-white">
                                <tr>
                                    <th className="px-6 py-3 text-left font-medium font-sans text-[14px] text-black uppercase tracking-wider">Staff Details</th>
                                    ...
                                    <th className="px-6 py-3 text-left font-medium font-sans text-[14px] text-black uppercase tracking-wider">Joined Date</th>
                                ...
                                {staff.data.length === 0 ? (
                                    <tr>
                                        <td colSpan="5" className="px-6 py-4 text-center text-gray-500">
                                            No staff members found. <Link href={route('admin.staff.create')} className="text-blue-600 hover:underline">Add your first staff member</Link>
                                        </td>
                                    </tr>
                                ) : (
                                    staff.data.map((item) => (
                                        <tr key={item.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="flex-shrink-0 h-10 w-10">
                                                        {item.photo ? (
                                                            <img className="h-10 w-10 rounded-full object-cover" src={`/storage/${item.photo}`} alt="" />
                                                        ) : (
                                                            <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500">
                                                                <span className="text-xl font-bold">{item.first_name.charAt(0)}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="ml-4">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-[14px] font-normal font-sans text-gray-900">{item.role}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item.status ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-center text-[14px] font-normal font-sans">
                                                <div className="flex items-center justify-center space-x-2">
                                                    <Link
                                                        href={route('admin.staff.edit', item.id)}
                                                    ...
                                                    <Link
                                                        href={route('admin.staff.destroy', item.id)}
                                                        method="delete"
                                                        as="button"
